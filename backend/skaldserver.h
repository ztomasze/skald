...
skaldServer : object 
     ...
     *   0 - None.
     *   1 - Errors, warnings, and essential info. 
    ...
    /** Start the server. */
    start() {
        if (self.LOG_LEVEL >= 1) "Starting server... ";
        self.server = new HTTPServer(getLocalIP(), self.port);
        if (self.LOG_LEVEL >= 1) "HTTP Server listening on port <<server.getPortNum()>>\n";
    ...
    sendReply(request, str) {
        local contents = skald.getHeader() + str.specialsToHtml() + skald.getFooter(); 
    ...
    processRequests() {        
        ...
        for (;;) {  //until we get a cmd
            ...
            local evt = getNetEvent();  //timeout in ms
            if (evt.evType == NetEvRequest && evt.evRequest.ofKind(HTTPRequest))
            {
                ...
                //a request for web file or other resource
                    ...
                    if (self.LOG_LEVEL >= 2) tadsSay('GET: <<query[1]>>\n');
                    if (query[1] == '/') {
                        query[1] = '/index.html';
                        if (self.LOG_LEVEL >= 3) tadsSay('GET converted: / -> /index.html\n');
                    }
